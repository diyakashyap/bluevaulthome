pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE = 'diya0311/bluevaulthome'
        KUBERNETES_DEPLOYMENT = 'home-frontend-deployment'
    }

    stages {
        stage('Clone Repository') {
            steps {
                git branch: 'master', url: 'https://github.com/diyakashyap/bluevaulthome.git'
            }
        }
        ...
        stage('Rolling Update Kubernetes Deployment') {
            steps {
                script {
                    ...
                    sh "kubectl set image deployment/$KUBERNETES_DEPLOYMENT home-frontend=$DOCKER_IMAGE:$TAG"
                }
            }
        }
    }
